...
const DomainBulkPopup = ({ BulkDomainUpdate, setIsBulkDomainOpen }: any) => {
    const [file, setFile] = useState<File>();

    const handleCreateDomain = async () => {
        try {
            console.log(file)
            const data = await BulkDomainUpdate(file);
        ...
        <main className="bg-black flex items-center justify-center h-screen">
            <div className="bg-white p-8 rounded-lg shadow-lg max-w-sm w-full">
                <div className="flex flex-col items-center">
                    <div className="text-2xl font-semibold mb-2">Upload Bulk Domain</div>
                </div>
                <div>
                    <div className="mb-4">
                    ...
                    <div className='flex'>
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <button
                                onClick={() => setIsBulkDomainOpen(false)}
                                className="bg-black text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                                type="button"
                            >
                                Cancel
...
export default DomainBulkPopup;
